...
void Painter::draw_str(const vec2 &p, const string &str) {
	...
	nix::set_shader(context->shader);
	nix::set_alpha_split(nix::Alpha::SOURCE_ALPHA, nix::Alpha::SOURCE_INV_ALPHA, nix::Alpha::ZERO, nix::Alpha::ONE);
	context->shader->set_color("_color_", _color);
	context->shader->set_default_data();
	nix::bind_texture(0, context->tex_text);
	nix::draw_triangles(context->vb);
	nix::disable_alpha();
}
...
void Painter::draw_rect(const rect &r) {
	if (fill) {
		...
		auto s = context->shader;
		if (corner_radius > 0) {
			s = context->shader_round;
